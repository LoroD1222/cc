...
import { getProject } from "@/sanity/projects";
...
const fallbackDetails: Record<string, { funding: string; agencies: string; partners: string; importance: string; progress: string; completion: string; location: string }> = {
...
export async function generateMetadata({ params }: ProjectDetailProps) {
  ...
  const cmsProject = await getProject(slug);
...
export default async function ProjectDetailPage({ params }: ProjectDetailProps) {
  ...
  const cmsProject = await getProject(slug);
  const fallbackProject = projects.find((item) => item.slug === slug);
  const fallbackDetail = fallbackProject ? fallbackDetails[fallbackProject.slug] : undefined;
  const project = cmsProject || fallbackProject;
  if (!project || (!cmsProject && !fallbackDetail)) notFound();

  const facts = cmsProject
    ? [["Countries involved", cmsProject.countries], ["Transport mode", cmsProject.type], ["Project status", cmsProject.status], ["Project cost", cmsProject.cost], ["Funding & financing gap", cmsProject.funding], ["Implementing agencies", cmsProject.implementingAgencies], ["Development partners", cmsProject.developmentPartners], ["Expected completion", cmsProject.expectedCompletion], ["Last update", cmsProject.lastUpdated]]
    : [["Countries involved", project.countries], ["Transport mode", project.type], ["Project status", project.status], ["Project cost", project.cost], ["Funding & financing gap", fallbackDetail!.funding], ["Implementing agencies", fallbackDetail!.agencies], ["Development partners", fallbackDetail!.partners], ["Expected completion", fallbackDetail!.completion], ["Last update", "Client input pending"]];
  ...
  return <>
    ...
    <main className="section"><div className="site-container project-detail"><div className="project-detail-image"><Image src={project.image} alt={cmsProject?.imageAlt || ""} fill preload sizes="(max-width: 900px) 100vw, 70rem" /></div><div className="detail-facts">{facts.map(([label, value]) => <article key={label}><strong>{label}</strong><span>{value}</span></article>)}</div><section className="detail-copy"><h2>Project description</h2>{cmsProject?.body.length ? <NewsArticleBody value={cmsProject.body} /> : <p>{project.description} This profile consolidates the available programme information for corridor stakeholders and will be updated as implementation data is confirmed.</p>}<h2>Strategic importance</h2>{cmsProject?.strategicImportance.length ? <NewsArticleBody value={cmsProject.strategicImportance} /> : <p>{fallbackDetail!.importance}</p>}<h2>Current progress</h2>{cmsProject?.currentProgress.length ? <NewsArticleBody value={cmsProject.currentProgress} /> : <p>{fallbackDetail!.progress}</p>}</section><section className="profile-resources"><div><MapPinned aria-hidden size={24} /><h2>Map & location</h2><p>{cmsProject?.locationLabel || fallbackDetail!.location}</p><Link className="text-link" href="/corridor">Open corridor map <ArrowRight aria-hidden size={16} /></Link></div><div><FileText aria-hidden size={24} /><h2>Photos & documents</h2>{cmsProject?.documents.length ? <ul>{cmsProject.documents.map((document) => <li key={document._id}>{document.url ? <a href={document.url} target="_blank" rel="noreferrer">{document.title}</a> : document.title}</li>)}</ul> : <p>Project images, procurement records and technical documents will be published here as available.</p>}<Link className="text-link" href="/resources">Browse resources <ArrowRight aria-hidden size={16} /></Link></div></section></div></main>
  </>;
}
