...
import { getNewsPage } from "@/sanity/news";
...
export default async function NewsPage({ searchParams }: { searchParams: Promise<{page?: string}> }) {
  ...
  const news = await getNewsPage(requestedPage(pageParam));
  ...
  return <>
    ...
    <section className="section"><div className="site-container news-layout"><div><NewsExplorer {...news} /></div><aside className="news-aside"><section><h2>Popular Highlights</h2>{["Dar es Salaam Port Expansion Masterplan (2026–2035)", "Burundi OSBP Border Post Delay Index Reduced", "Single Window Customs Integration Updates"].map((title) => <article key={title}><span>Highlight</span><h3>{title}</h3></article>)}</section><section className="bulletin-card"><h2>Corridor Bulletin</h2><p>Subscribe to receive monthly regional logistics observatory updates and active tender announcements.</p><label className="field"><span className="sr-only">Email</span><input type="email" placeholder="your.email@domain.com" /></label><button className="button" type="button">Subscribe</button></section></aside></div></section>
